...
class RemotePeerContext extends AbstractPeerContext implements ClusterSingletonService, AutoCloseable {
        ...
        ask(masterActorRef, new InitMasterMountPoint(rpcDataBroker, rpcBridge), Timeout.apply(10, TimeUnit.SECONDS))
                .onComplete(new OnComplete<>() {
                    ...
                    public void onComplete(final Throwable failure, final Object success) {
                        if (failure == null) {
                            publishState(new ActualEndpointsBuilder(peer), MountStatus.Mounted, Optional.empty(),
                                    selfAddress);
                        } else {
    ...
    public String toString() {
        return "RemotePeerContext [name=" + name + ", closed=" + closed + ", stopped=" + stopped + ", master=" + master
                + "]";
    }
}
