...
class FailedProxyTransaction implements ProxyTransactionFacade {
    ...
    public FluentFuture<Optional<NormalizedNode<?, ?>>> read(final LogicalDatastoreType store,
        ...
        return FluentFutures.immediateFailedFluentFuture(mapFailure("read"));
    }

    @Override
    public FluentFuture<Boolean> exists(final LogicalDatastoreType store, final YangInstanceIdentifier path) {
        LOG.debug("[{}][FAILED] Exists {} {}", name, store, path, failure);
        return FluentFutures.immediateFailedFluentFuture(mapFailure("exists"));
    }
    ...
    private ReadFailedException mapFailure(String msg) {
        return ReadFailedException.MAPPER
                .apply(failure instanceof Exception ? (Exception) failure : new ReadFailedException(msg, failure));
    }
}