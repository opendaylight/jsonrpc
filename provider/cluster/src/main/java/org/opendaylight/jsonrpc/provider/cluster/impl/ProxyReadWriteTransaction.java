...
class ProxyReadWriteTransaction implements DOMDataTreeReadWriteTransaction {
    ...
    private volatile ProxyTransactionFacade facade;
    ...
    public ProxyReadWriteTransaction(final Peer peer, final Future<Object> future,
        ...
        future.onComplete(new OnComplete<>() {
            ...
            public void onComplete(final Throwable failure, final Object actorRef) {
                final ProxyTransactionFacade txFacade;
                if (failure != null) {
                    LOG.debug("[{}] Failed to obtain master actor", name, failure);
                    txFacade = new FailedProxyTransaction(name, failure);
                } else {
                    LOG.debug("[{}] Obtained master actor {}", name, actorRef);
                    txFacade = new ActorProxyTransaction((ActorRef) actorRef, peer, executionContext, askTimeout);
                }

                invokeBefore(txFacade);
            }
        }, executionContext);
    }
