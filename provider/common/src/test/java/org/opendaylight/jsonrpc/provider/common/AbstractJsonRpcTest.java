 ...
 */
public abstract class AbstractJsonRpcTest extends AbstractConcurrentDataBrokerTest {
    ...
    private BindingDOMCodecServices bnnc;

    protected AbstractJsonRpcTest() {
        super(true);
    }

    @Override
    protected void setupWithRuntimeContext(final BindingRuntimeContext runtimeContext) {
        bnnc = ServiceLoader.load(BindingDOMCodecFactory.class).findFirst().orElseThrow()
            .createBindingDOMCodec(runtimeContext);
    ...
    protected void setupWithSchema(final EffectiveModelContext context) {
        ...
        rpcRouter = new DOMRpcRouter(getDataBrokerTestCustomizer().getSchemaService());
