...
public class RemoteControlProvider
    ...
    public RemoteControlProvider(@NonNull ProviderDependencies dependencies) {
        ...
        // TODO : Read initial data and configure services?
    }
    ...
    private boolean resetGovernance(final Uri rootOm) throws URISyntaxException {
        if (rootOm != null) {
            if (!rootOm.getValue().equals(governanceRootUri)) {
                governanceRootUri = rootOm.getValue();
                stopGovernance();
                LOG.debug("(Re)setting governance root to {}", rootOm.getValue());
