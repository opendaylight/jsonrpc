...
public class PubSubTest extends AbstractSessionTest {
    ...
    @Test(timeout = 15_000)
    public void testDefaultTopic() throws InterruptedException {
    ...
    @Test(timeout = 15_000)
    public void testNonDefaultTopic() throws InterruptedException {
        final int port = getFreeTcpPort();
        final CountDownLatch latch = new CountDownLatch(1);
        final Subscriber subscriber = factory.subscriber(getConnectUri(port), "topic1", new MessageListener() {
            @Override
            public void onMessage(PeerContext peerContext, String message) {
                LOG.info("Response : {}", message);
                latch.countDown();
        ...
        final Publisher publisher = factory.publisher(getBindUri(port));
        subscriber.awaitConnection();
        publisher.publish("TEST", "topic1");
        // this will not be delivered
        publisher.publish("TEST", "topic2");
        assertTrue(latch.await(10, TimeUnit.SECONDS));
        publisher.close();
        subscriber.close();
    }
